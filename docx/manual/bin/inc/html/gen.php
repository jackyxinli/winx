<?php
	...
	*/
	...
	function genSvrHpp($server, $indent)
	{
		...
		echo "\n${indent}class $impl_class_name : public ${class_name}${class_postfix_base} \n";
		echo "${indent}{\n";
		echo "${indent}protected:\n";
		$indent2 = $indent . "\t";
			echo "${indent2}cerl::Process* const _self;\n";
	...
	{
		echo "\n";
		echo "${indent}static cerl::PID cerl_call _start(";
		if (count($ctor))
			echo "\n${indent}\t";
	...
	function genCtor($class_name, $ctor, $indent)
	{
		...
		if (!count($ctor) || !(@$ctor->args))
			echo "explicit ";
		echo "${class_name}(cerl::Process* _me";
